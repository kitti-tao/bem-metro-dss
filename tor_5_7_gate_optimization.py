...
def gate_optimization(event_data, event_context):
  print('start')

  # get dataframe from BigQuery
  client, usage = bq_to_dataframe(query=gate_usage_query)
  ...
  # define constraint
  def gate_constraint(a):
    if a > 0:
      return True

  problem.addConstraint(gate_constraint)

  # find objective function
  solutions = problem.getSolutions()
  usage['NextYearUsage'] = usage['GrowthRate']*usage['Usage']
  ...
  for s in solutions:
    ...
    usage['next_year_optimized_quantity'] = usage.apply(lambda x: s['a'] if x.Percentage/100*15*x.Throughput*s['a']- x.NextYearUsage >= 0 else x.next_year_optimized_quantity, axis=1)

  # save result to BigQuery
  result_to_bq(usage, client, dataset_id='datamart_for_report', dataset_table="report_5_7_0_gate_optimization_result_table")
  print('finish')
